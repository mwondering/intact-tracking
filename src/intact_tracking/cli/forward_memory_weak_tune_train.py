"""Fine-tune u8000 to u10000 with doubled weak weights and response scale 0.3."""
...
from intact_tracking.memory350_weak_tuning import (
    MILESTONES, TUNED_LOSS, validate_tuning_arguments, validate_tuning_losses,
)
...
def build_parser():
    ...
    parser.set_defaults(**TUNED_LOSS, stop_after_updates=MILESTONES[-1])
    return parser
...
def reference_contract(reference, actual):
    ...
    actual["method"] = "nominal50 Memory350 encoder2x u8000 joint weak-pair tuning"
    ...
    for module in (Path(__file__), Path(weak.__file__),
                   Path(__file__).parents[1] / "memory350_weak_tuning.py"):
        actual["research_source_sha256"][str(module.resolve().relative_to(Path(__file__).resolve().parents[3]))] = trainer._sha256(module)
    result.update(architecture_change="none", initialization="restore u8000 model and AdamW state",
...
def main():
    ...
    if not args.resume or args.bounded_smoke:
        raise ValueError("Use this entry only for the explicit u8000 tuning branch")
