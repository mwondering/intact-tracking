"""Train original Memory350 with 50% nominal A and 50% tracker DR plus limb loads."""
...
from intact_tracking.memory350_model import (
    Memory350Predictor as ForwardDynamicsTransformer,
    Memory350Config as ForwardPredictorConfig,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.set_defaults(num_envs=8192, validation_worlds=128, seed=717,
                        ...
                        context_history_steps=50, representation_relation_weight=2.0,
...
def _validate_arguments(args: argparse.Namespace) -> None:
    ...
    if args.context_history_steps != 50 or not args.tracker_dr_plus_limb_payload or args.limb_payload_only:
        raise ValueError("Memory350 fixes short50 and frozen-tracker DR plus four-limb loads")
    positive = (
        "num_envs",
        "warmup_steps",
        "max_warmup_steps",
...
def _wandb_payload(record: dict[str, Any]) -> dict[str, Any]:
    ...
    payload.update({f"memory/{name}": value for name, value in record.get("memory", {}).items()})
    return payload
...
def _save_checkpoint(
    *,
    ...
    state = {
        ...
        "memory_contract": "short50_disjoint_raw_chunk10_long30_one_memory_latent_v1",
...
def _run(args: argparse.Namespace, distributed: DistributedContext) -> Path:
    ...
    run_config = {
        ...
        "research_source_sha256": {
            str(p.relative_to(Path(__file__).resolve().parents[3])): _sha256(p) for p in (
                ...
                Path(__file__).resolve().parents[1] / "memory350_nominal_rollout.py",
                Path(__file__).resolve().parents[1] / "memory350_model.py",
                Path(__file__).resolve().parents[1] / "memory350_objective.py",
                Path(__file__).resolve().parents[1] / "memory350_replay.py",
                Path(__file__).resolve().parents[1] / "memory350_bank.py",
    ...
    run_config["method"] = "nominal50-supervised hierarchical short50/chunk10/long30 context v1"
    run_config["architecture"]["context"] = (
        "50 within-trial completed interactions plus one 128-D memory token and CLS (52 tokens); "
        "memory token attends over 30 shared-encoder summaries of reset-free 10-interaction chunks; "
        "raw long and short histories are disjoint, short updates first, reset flushes full chunks "
        "and discards incomplete tails; the final context is 64-D")
    run_config["memory_contract"] = {
        "short_steps": 50, "chunk_steps": 10, "long_chunks": 30, "long_summary_tokens": 1,
        "maximum_represented_interactions": 350, "short_long_overlap": False,
        "storage": "raw transitions, re-encoded with current parameters on each optimizer forward",
        "replay_query_causality": "query-time committed chunk counters; no future chunks",
        "reset": "flush full old-trial chunks, drop incomplete tail and invalid reset edge, clear short",
        "physical_parameter_change": "invalidate affected world's short, pending and long memory",
        "representation_eligibility": "any valid short interaction or long chunk; full short history is not required",
    }
        ...
        for update in training_update_indices(first_update, args.updates, args.until_user_stop):
            ...
            if distributed.is_main:
                _write_json(output_dir / "progress.json", {"completed_updates": update,
                    "optimizer_steps": optimizer_steps, "unix_time": time.time(), "unbounded": args.until_user_stop})
                if not should_report and update % args.log_interval == 0:
                    light = {"update": update, "optimizer_steps": optimizer_steps,
                             "learning_rate_model": optimizer.param_groups[0]["lr"]}
                    for name in ("loss", "prediction_loss", "representation_loss", "gradient_norm"):
