"""Train the matched short50-only nominal-supervised control from fresh initialization."""
...
from intact_tracking.short50_model import (
    Short50Predictor as ForwardDynamicsTransformer,
    Short50Config as ForwardPredictorConfig,
)
from intact_tracking.short50_comparison import reference_contract, reference_normalization, reference_probes
...
def _validate_arguments(args: argparse.Namespace) -> None:
    ...
    if args.context_history_steps != 50 or not args.tracker_dr_plus_limb_payload or args.limb_payload_only:
        raise ValueError("Short50 control fixes short50 and frozen-tracker DR plus four-limb loads")
    positive = (
        "num_envs",
        "warmup_steps",
        "max_warmup_steps",
...
def _wandb_payload(record: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["model/long_memory_enabled"] = False
    return payload
...
def _save_checkpoint(
    *,
    ...
    state = {
        ...
        "memory_contract": "short50_only_within_episode_no_long_memory_v1",
        "long_memory_in_model": False,
...
def _run(args: argparse.Namespace, distributed: DistributedContext) -> Path:
    ...
    run_config["method"] = "nominal-supervised short50-only matched control v1"
    run_config["architecture"]["context"] = (
        "50 within-episode completed interactions plus CLS (51 tokens); no memory token "
        "or long-memory encoder; fresh shared weights/positions match Memory350 initialization")
    run_config["memory_contract"] = {
        "short_steps": 50, "long_chunks_in_model": 0, "long_summary_tokens": 0,
        "maximum_represented_interactions": 50, "cross_reset_model_history": False,
        "reset": "short history is cleared and padded after reset or motion discontinuity",
        "collector": "same Memory350 replay to preserve sample/positive-pair selection; long data never enters model",
    ...
    run_config["research_source_sha256"].update({
        ...
        for p in [*sorted(Path(__file__).resolve().parents[1].glob("memory350_*.py")),
                  *sorted(Path(__file__).resolve().parents[1].glob("short50_*.py"))]})
    ...
    try:
        ...
        for update in training_update_indices(first_update, args.updates, args.until_user_stop):
            ...
            if should_report:
                ...
                if args.comparison_reference_dir:
                    ...
                    if reference_row is not None:
                        ...
                        matched_comparison = {
                            ...
                            "short50_dr_five_step_nmse": fixed_probe["dr_five_step_nmse"],
                            "short50_to_memory350_nmse_ratio": fixed_probe["dr_five_step_nmse"] / ref_nmse,
                        }
                ...
                for label, prefix in (("final_encoder", "context_encoder.transformer"),):
                ...
                record = {
                    ...
                    "model_long_memory_enabled": False,
