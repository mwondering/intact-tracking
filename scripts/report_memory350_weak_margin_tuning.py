"""Report the u10000 baseline and u11000/u12000 checks toward DR+load Top-1 80%."""
...
def summarize(root):
    ...
    for update in plan['evaluation_updates']:
        ...
        if path.exists():
            ...
            if summary.get('complete'):
                ...
                rows.append(row)
    result = {'checks': rows, 'reference_update': plan['from_update'], 'scheduled_updates': plan['evaluation_updates'],
              'primary_metric': plan['primary_metric'], 'loss_changes': plan['loss_changes'],
              'primary_profile': 'memory_training', 'target_top1': plan['target_top1'],
    ...
    lines = [
        ...
        '|---|---:|---:|---:|']
    for row in rows:
        lines.append(f'| {row["update"]} | {row["update"] - plan["from_update"]} | {100 * row["common_top1"]:.2f}% | {100 * row["native_top1"]:.2f}% |')
    ...
    lines += ['', '这轮测量三项修改与继续训练的联合效果，不能拆分各项参数的独立贡献。', '',
              f'[父模型 u10000 检查]({Path(plan["parent_evaluation_summary"]).with_name("review.md")}) · [趋势数据](trend.json) · [启动配置](launch_contract.json)']
    for row in rows[1:]:
    ...
    for ax, title, curves in zip(axes, [
        ...
        ax.set(title=title, xlabel='Total updates', xticks=[plan['from_update'], *plan['evaluation_updates']])
        if ax is axes[0]:
            ax.axhline(plan['target_top1'], color='grey', linestyle='--', linewidth=1, label='80% target')
        ax.grid(alpha=.25)
