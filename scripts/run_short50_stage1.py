"""Launch matched Short50 on GPU 0–3 alongside Memory350; keep 8192 environments per rank."""
...
def command_for(output, num_envs, smoke):
    command = [PYTHON, "-u", "-m", "torch.distributed.run", "--standalone", "--nproc-per-node=4",
               "--max-restarts=0", "-m", "intact_tracking.cli.forward_short50_train",
               "--checkpoint-file", TRACKER, "--output-dir", str(output), "--num-envs", str(num_envs),
...
def baseline_unchanged():
    manifest = ROOT / ".runtime/short50_v1/protected_sources.json"
...
def existing_workers(root):
    ...
    for process in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        if "intact_tracking.cli.forward_short50_train" not in argv or "--output-dir" not in argv:
            continue
...
def poll_evaluation(root, output, environment, active):
    if active is not None:
        process, handle, update = active
        if process.poll() is None:
            return active
        handle.close()
        write_json(root / "comparison" / f"update_{update:06d}" / "process_result.json",
                   {"update": update, "exit_code": process.returncode, "finished_at": time.time()})
    for update in (100, 500, 1000, 3000, 5000, 7500):
        destination = root / "comparison" / f"update_{update:06d}"
        if (destination / "process_result.json").exists():
            continue
        short = output / f"update_{update:06d}.pt"
        memory = REFERENCE / f"update_{update:06d}.pt"
        if not short.exists() or not memory.exists():
            continue
        destination.mkdir(parents=True, exist_ok=True)
        env = dict(environment)
        env.update(CUDA_VISIBLE_DEVICES="0", WANDB_RUN_ID="s50cmp-" + hashlib.sha256(str(destination).encode()).hexdigest()[:12],
                   WANDB_DIR=str(destination))
        command = [PYTHON, "-B", "-u", str(ROOT / "scripts/evaluate_short50_comparison.py"),
                   "--short-checkpoint", str(short), "--memory-checkpoint", str(memory),
                   "--output", str(destination), "--wandb", "--group", root.name + "-paired-evaluation"]
        handle = (destination / "evaluation.log").open("w")
        process = subprocess.Popen(command, cwd=ROOT, env=env, stdout=handle, stderr=subprocess.STDOUT,
                                   start_new_session=True)
        write_json(destination / "process.json", {"pid": process.pid, "update": update, "started_at": time.time()})
        return process, handle, update
    return None
...
def run_job(root, output, num_envs, smoke, history):
    ...
    env.update(CUDA_VISIBLE_DEVICES="0,1,2,3", PYTHONUNBUFFERED="1",
               ...
               WANDB_RUN_ID="s50-" + hashlib.sha256(str(output).encode()).hexdigest()[:12],
               WANDB_RESUME="allow")
    ...
    with log.open("w") as handle:
        ...
        while child.poll() is None:
            if not smoke:
                evaluation = poll_evaluation(root, output, env, evaluation)
            write_json(root / "state.json", {"status": "running", "launcher_pid": os.getpid(), "job": record,
                                              "attempts": history, "heartbeat": time.time()})
            time.sleep(5)
...
def run(root):
    ...
    with (root / ".launcher.lock").open("w") as lock:
        ...
        write_json(root / "launch_contract.json", {
            "gpus": [0, 1, 2, 3], "environments_first": 8192, "environments_after_confirmed_oom": None,
            "dataset": DATASET, "maximum_updates": None, "stage2_jobs": [], "paired_reference": str(REFERENCE),
            "resource_plan": "co-located on GPU 0-3 with existing Memory350; do not stop its workers",
            ...
            "new_sources": {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                            for p in [ROOT / "src/intact_tracking/cli/forward_short50_train.py",
                                      *sorted((ROOT / "src/intact_tracking").glob("memory350_*.py")),
                                      *sorted((ROOT / "src/intact_tracking").glob("short50_*.py"))]},
        })
        history = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-root", type=Path, default=ROOT / "runs/limb_context_20260910_short50")
