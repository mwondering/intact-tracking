...
def command_for(output, num_envs, smoke):
    command = [PYTHON, "-u", "-m", "torch.distributed.run", "--standalone", "--nproc-per-node=4",
               "--max-restarts=0", "-m", "intact_tracking.cli.forward_memory_train",
...
def existing_workers(root):
    ...
    for process in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        if "intact_tracking.cli.forward_memory_train" not in argv or "--output-dir" not in argv:
            continue
...
def run_job(root, output, num_envs, smoke, history):
    ...
    env.update(CUDA_VISIBLE_DEVICES="0,1,2,3", PYTHONUNBUFFERED="1",
               ...
               WANDB_RUN_ID="m350-" + hashlib.sha256(str(output).encode()).hexdigest()[:12],
               WANDB_RESUME="allow")
...
def run(root):
    ...
    with (root / ".launcher.lock").open("w") as lock:
        ...
        write_json(root / "launch_contract.json", {
            ...
            "dataset": DATASET, "maximum_updates": None, "stage2_jobs": [],
            ...
            "new_sources": {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                            for p in [ROOT / "src/intact_tracking/cli/forward_memory_train.py",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-root", type=Path, default=ROOT / "runs/limb_context_20260909_memory350")
