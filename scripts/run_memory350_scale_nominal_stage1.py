"""Train nominal50 encoder2x on the user-assigned GPUs 4-7 with matched original controls."""
...
REFERENCE = ROOT / "runs/limb_context_20260911_memory350_nominal50/stage1_8192"
...
def available(cards):
    return all(card["free_mib"] >= 60000 for card in cards)


def command_for(output, smoke, reference=REFERENCE):
    command = [PYTHON, "-B", "-u", "-m", "torch.distributed.run", "--standalone", "--nproc-per-node=4",
        "--max-restarts=0", "-m", "intact_tracking.cli.forward_memory_scale_nominal_train",
        "--nominal-fraction", "0.5", "--checkpoint-file", TRACKER, "--output-dir", str(output), "--num-envs", "8192",
        ...
        "--wandb-name", "memory350_encoder2x_nominal50_" + ("smoke" if smoke else "seed717")]
    if smoke:
...
def live_workers(root):
    ...
    for process in Path("/proc").iterdir():
        ...
        try:
            ...
            if "intact_tracking.cli.forward_memory_scale_nominal_train" in argv and "--output-dir" in argv:
...
def poll_evaluation(root, output, reference, env, gpu, active):
    ...
    for update in MILESTONES:
        ...
        child_env = dict(env, CUDA_VISIBLE_DEVICES=str(gpu),
                         WANDB_RUN_ID="m350scale-n50-eval-" + hashlib.sha256(str(folder).encode()).hexdigest()[:12])
...
def run_job(root, output, smoke, reference, gpus):
    ...
    env.update(CUDA_VISIBLE_DEVICES=",".join(map(str, gpus)), PYTHONUNBUFFERED="1",
        ...
        WANDB_RUN_ID="m350scale-n50-" + hashlib.sha256(str(output).encode()).hexdigest()[:12], WANDB_RESUME="allow")
    ...
    if not smoke:
        (root / 'stage1').symlink_to(output.name, target_is_directory=True)
    ...
    while child.poll() is None:
        ...
        if not smoke and not (root / 'STOP').exists():
    ...
    while not smoke and not (root / 'STOP').exists():
        evaluation = poll_evaluation(root, output, reference, env, gpus[0], evaluation)
        if evaluation is None:
            break
        time.sleep(10)
    completion = read_json(output / "completion.json")
    if child.returncode != 0 and not (completion and completion.get('stopped') and (root / 'STOP').exists()):
...
def run(args):
    ...
    if len(set(gpus)) != 4 or any(gpu not in (4, 5, 6, 7) for gpu in gpus):
        raise ValueError("This experiment is assigned physical GPUs 4-7")
    ...
    with (root / ".launcher.lock").open("w") as lock:
        ...
        contract = {"gpus": gpus, "num_envs_per_rank": 8192, "dataset": DATASET,
            ...
            "resource_policy": "user assigned GPUs 4-7; share with existing jobs when each has at least 60000 MiB free; never stop unrelated jobs",
            "nominal_a_fraction": 0.5, "initialization": "from scratch",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-root", type=Path, default=ROOT / "runs/limb_context_20260911_memory350_encoder2x_nominal50")
