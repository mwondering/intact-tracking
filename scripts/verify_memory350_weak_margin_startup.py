"""Verify the live u10000 margin tuning branch, restored state and fixed validation."""
...
from intact_tracking.memory350_weak_margin_tuning import (
    MILESTONES, PARENT_UPDATE, TUNED_LOSS, FIXED_LOSS, validate_tuning_arguments, validate_tuning_losses,
)
...
def verify(root):
    ...
    assert progress and progress['completed_updates'] >= PARENT_UPDATE + 10
    assert progress['optimizer_steps'] == progress['completed_updates'] * 4
    assert progress['stop_after_updates'] == MILESTONES[-1]
    validate_tuning_arguments(previous['arguments'], config['arguments'], PARENT_UPDATE)
    ...
    assert resume['completed_update'] == PARENT_UPDATE and resume['optimizer_steps'] == 4 * PARENT_UPDATE
    ...
    first = rows[0]
    assert first['update'] == PARENT_UPDATE + 1 and first['optimizer_steps'] == 4 * (PARENT_UPDATE + 1)
    ...
    expected = (TUNED_LOSS['weak_positive_weight'] * train['weak_positive_loss']
                + TUNED_LOSS['weak_negative_weight'] * train['weak_negative_loss'])
    ...
    assert all(config['loss'][key] == value for key, value in (TUNED_LOSS | FIXED_LOSS).items())
