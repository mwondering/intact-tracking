"""Verify nominal50 encoder2x on GPUs 4-7 with original nominal50 validation and normalization."""
...
def main():
    ...
    original = json.loads((project / 'runs/limb_context_20260911_memory350_nominal50/stage1_8192/run_config.json').read_text())
    ...
    assert config['model'] == {**original['model'], 'chunk_depth': 2, 'memory_depth': 4, 'context_depth': 4}
    assert config['parameter_counts'] == dict(total=21086486, context_encoder=2026688, predictor=19059798)
    ...
    same_arguments = ('motion_path', 'seed', 'batch_size', 'micro_batch_size', 'warmup_steps',
                      ...
                      'context_history_steps', 'context_dim',
    ...
    assert len(ranks) == 4
    assert {r['physical_gpu'] for r in ranks} == {'4', '5', '6', '7'}
    counts = []
    for rank in ranks:
    ...
    reference = Path(config['matched_control']['reference_dir'])
    assert json.loads((run / 'normalization.json').read_text()) == json.loads((reference / 'normalization.json').read_text())
    ...
    for rank in range(4):
        ...
        for prefix in ('validation', 'validation_broad'):
            ...
            assert hashlib.sha256((run / name).read_bytes()).hexdigest() == hashlib.sha256((reference / name).read_bytes()).hexdigest()
    ...
    assert all(first['collector_memory'][f'gradient_norm_{name}'] > 0
    ...
    result = {'passed': True, 'run': str(run), 'progress_at_verification': progress,
              'checkpoint_update': checkpoint['update'], 'only_attention_depths_changed': True, 'parameter_counts': config['parameter_counts'],
              'shared_normalization_and_eight_validation_files': True,
