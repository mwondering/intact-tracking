"""Prepare and supervise the authorized u8000 three-loss tuning branch on GPUs 4-7."""
...
from intact_tracking.memory350_weak_tuning import MILESTONES, PARENT_UPDATE, TUNED_LOSS


def tuning_command(root, parent):
    command = list(read_json(parent / 'launch_contract.json')['formal_command'])
    old_module = 'intact_tracking.cli.forward_memory_weak_pairs_train'
    command[command.index(old_module)] = 'intact_tracking.cli.forward_memory_weak_tune_train'
...
def prepare(root, parent):
    import torch
    from intact_tracking.cli.forward_memory_weak_tune_train import build_parser, reference_contract
    ...
    evaluation_path = parent / f'continuation_005000_010000/comparison_{PARENT_UPDATE:06d}/summary.json'
    evaluation = read_json(evaluation_path)
    if not evaluation or not evaluation.get('complete'):
        raise RuntimeError('The original u8000 evaluation must finish before tuning is prepared')
    ...
    index = command.index('intact_tracking.cli.forward_memory_weak_tune_train')
    ...
    assert state['scheduler']['T_max'] == state['scheduler']['last_epoch'] == 32000
    assert state['optimizer']['param_groups'][0]['lr'] == args.continuation_min_learning_rate == 1e-5
    assert all(int(value['step'].item()) == 32000 for value in state['optimizer']['state'].values())
    ...
    sources = sorted((ROOT / 'src/intact_tracking').rglob('*.py')) + [
        ...
        ROOT / 'scripts/report_memory350_weak_tuning.py']
...
def refresh_report(root, env):
    with (root / 'report.log').open('a') as log:
        subprocess.run([PYTHON, '-B', str(ROOT / 'scripts/report_memory350_weak_tuning.py'),
...
def verify_completion(root, plan):
    ...
    assert completion['completed_updates'] == 10000 and completion['optimizer_steps'] == 40000 and completion['hit_cap']
    paths = {'baseline': Path(plan['parent_checkpoint']), 'memory350': output / 'update_010000.pt'}
    ...
    assert all(int(value['step'].item()) == 40000 for value in states['memory350']['optimizer']['state'].values())
...
def main():
    ...
    with (root / '.launcher.lock').open('a') as lock:
        ...
        if any(card['free_mib'] < 60000 for card in cards):
            raise RuntimeError('The previous training must stop before starting tuning on GPUs 4-7')
        ...
        env.update(CUDA_VISIBLE_DEVICES='4,5,6,7', PYTHONUNBUFFERED='1',
                   ...
                   WANDB_RESUME='never', WANDB_RUN_ID='m350tune-' + hashlib.sha256(str(root).encode()).hexdigest()[:12])
        refresh_report(root, env)
